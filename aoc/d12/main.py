...
def read_file(input_file):
    ...
    directions = [list(regex_instr.match(x).groups()) for x in input_file.readlines()]
...
def interpret_dir(start_coord, start_dir, direction):
    ...
    if dir_ == 'F':
        ...
        end_coord = add_tuple(start_coord, mult_tuple((0,1), num_))
    elif dir_ == 'S':
        end_coord = add_tuple(start_coord, mult_tuple((0,-1), num_))
    elif dir_ == 'E':
        end_coord = add_tuple(start_coord, mult_tuple((1, 0), num_))
    elif dir_ == 'W':
        end_coord = add_tuple(start_coord, mult_tuple((-1,0), num_))
    elif dir_ == 'L':
        num_rad = num_*math.pi/180
        end_dir = (int(math.cos(current_angle+num_rad)), \
            int(math.sin(current_angle+num_rad)))
...
def interpret_dir_waypoint(coord_, waypoint, direction):
    ...
    #moves relative direction of waypoint relative to ship
        ...
        waypoint = add_tuple(waypoint, mult_tuple((0,1), num_))
    elif dir_ == 'S':
        waypoint = add_tuple(waypoint, mult_tuple((0,-1), num_))
    elif dir_ == 'E':
        waypoint = add_tuple(waypoint, mult_tuple((1, 0), num_))
    elif dir_ == 'W':
        waypoint = add_tuple(waypoint, mult_tuple((-1,0), num_))
    #moving the waypoint around the ship, ship in meme place
        ...
        new_waypoint_ship_vector = (math.cos(waypoint_ship_angle+num_rad), \
            math.sin(waypoint_ship_angle+num_rad))
...
def p_2(input_file: IO,
    ...
    for direction in directions:
        coord_, waypoint_coord = interpret_dir_waypoint(coord_, waypoint_coord, direction)
        print(direction, coord_, waypoint_coord)
    return get_manhattan_distance(coord_)
